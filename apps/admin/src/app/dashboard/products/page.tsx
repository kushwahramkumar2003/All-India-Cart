...
import { getAllSupplireProducts } from '@/services/product';
...
export default function Page(): React.JSX.Element {
  ...
  const { data, error, isError, isPending, refetch } = useQuery({
    queryKey: ['products'],
    queryFn: async () => {
      return await getAllSupplireProducts({ query: search });
    },
  });
  if (data) {
    console.log('products in a server ', data);
  }
    ...
    return (
      <div role="alert ">
        ...
        <div className="border border-t-0 border-red-400 rounded-b bg-red-100 px-4 py-3 text-red-700">
          <p>Error in fetching the data to the server..</p>
        </div>
      </div>
    );
  }
  return (
    <div>
      ...
      <DataTable products={data as Product[]} refetch={refetch} />
    </div>
  );
}
